...
NBR_SNAKES_IN_GENERATION = 300				# Nombre de snakes dans une génération
BEST_SNAKES = []							# Liste des meilleurs snakes de la génération précédente
MUTATION_RATE = 1							# Pourcentage de mutation à chaque création d'un noeud pour un nouveau snake (à partir de la deuxième GENERATION) 
											# /!\ la mutation est appelée 156 fois par snake (à chaque poids). Pour une valeur de 1, il y aura ~1,5 poids de mutés par snake /!\
...
class Snake(object):
	...
	def calculate_performance(self, steps):
		if steps == MAX_STEPS :
			...
			return int(((2*steps) + (2**eaten_apples) + (450*(eaten_apples**2.1)) - (0.3*(steps**1.3)*(eaten_apples**1.2))))
...
def get_random_weights():
	global BEST_SNAKES
	a = randrange(0,NBR_SNAKES_TO_REPRODUCE,1)
	b = randrange(0,NBR_SNAKES_TO_REPRODUCE,1)
	WB_list = []
	sublist = []
	for i in range(0,6) :
		for j in range(0,16) :
			if is_there_mutation(MUTATION_RATE) :
				...
				try:
					sublist.append(str2array(BEST_SNAKES[choice([a,b])]["Wb"])[i][j])
				except:
	...
	for k in range(0,6) :
		for l in range(0,6) :
			if is_there_mutation(MUTATION_RATE) :
				...
				try:
					sublist2.append(str2array(BEST_SNAKES[choice([a,b])]["Wc"])[k][l])
				except:
	...
	for m in range(0,4) :
		for n in range(0,6) :
			if is_there_mutation(MUTATION_RATE) :
				...
				try:
					sublist3.append(str2array(BEST_SNAKES[choice([a,b])]["Wd"])[m][n])
				except:
...
def go_next_generation():
	global GENERATION, BEST_SNAKES
	BEST_SNAKES = get_best_snakes(GENERATION, NBR_SNAKES_TO_REPRODUCE)
	GENERATION = GENERATION + 1
	create_new_log_file()

# Trie retourne la valeur "performance" d'un snake
def return_performance(value):

    return value['performance']

# Retourne une liste des meilleurs snakes d'une génération
def get_best_snakes(generation, nbr_snakes):
	fichier = open(dir_path+"/GENERATION"+str(generation)+".json", 'r')
	jsonn = json.load(fichier)
	fichier.close()
	top_snakes = sorted(jsonn['snakes'], key=return_performance,reverse=True)
	return top_snakes[0:NBR_SNAKES_TO_REPRODUCE]
...
def create_new_log_file():
	with open(dir_path+"/GENERATION"+str(GENERATION)+".json", 'w+',encoding='utf-8') as file:
		file.write('{"snakes": []}')
		file.close()
...
def log_snakes():
	...
	jsonn = json.load(fichier)
	fichier.close()
	fichier = open(dir_path+"/GENERATION"+str(GENERATION)+".json", 'w+')
...
def lose():
	...
	STEPS = 0
	SNAKE_ID = SNAKE_ID + 1
	if SNAKE_ID == NBR_SNAKES_IN_GENERATION :
		if len(LOG_SNAKES) > 0 :
			log_snakes()
			LOG_COUNT = 0
			LOG_SNAKES = []
...
def newGame():
	global board, snake, DIRECTION, LAST_QUEUE, LAST_HEAD, ACTUAL_NEURAL, LOST_PARAM
	print("-----------------------------\n")
	LOST_PARAM = False
	snake=Snake()
	...
	LAST_QUEUE = "0x0"
	if GENERATION == 0 :
		ACTUAL_NEURAL = Neural(random_inputs=True)
	else :
		ACTUAL_NEURAL = Neural(random_inputs=False)

def Game():
	global board, DIRECTION, snake, ACTUAL_NEURAL, STEPS, LOST_PARAM
...
def main():
	global snake,fenetre,board,BEST_SNAKES,LOST_PARAM
	create_new_log_file()
	snake=Snake()
	fenetre = tkinter.Tk()
	...
	if GENERATION == 0 :						# Si GENERATION != 0, on indique de ne pas utiliser de poids random 
		pass
	else :
		BEST_SNAKES = get_best_snakes(GENERATION-1, NBR_SNAKES_TO_REPRODUCE)

	while True :
		newGame()
		while not LOST_PARAM :
			Game()
		lose()
